...
def komenda():  # Main menu for commands
    ...
    polecenie = input('> ')
    dozwolone_polecenia = ['idź', 'badaj', 'zbadaj', 'wyjdź', 'popatrz', 'rusz się', 'biegnij', 'opisz', 'misje',
                           'misja', 'wykonaj misje', 'skontaktuj się z nasa', 'skontaktuj', 'przygotuj', ]
    while polecenie.lower() not in dozwolone_polecenia:
        print_wolno('''Proszę wpisać poprawną komendę:
...
def aktualna_misja():  # Def for printing actual mission
    if gracz.misje_wykonane == 0:
...
def gracz_ruch():  # Def for player movement
...
def gracz_opis():  # Def for "opisz", in english: 'review' or 'description'
    print_wolno((mapa[gracz.lokalizacja][Opis]))
    time.sleep(1)
    komenda()


def gracz_badanie():  # Def for "zbadaj", in english: "examine"
    if gracz.misje_wykonane == 0:
...
def koniec_gry():  # Def for game over
    print_wolno('Ukończyłeś grę, a przynajmniej tę ścieżkę historii')
    time.sleep(2)
    sys.exit()
...
Wybrane opcje wpływają na historię.\n''')
    print('\n')
    print_wolno(('''Nazywasz się Elon, pracujesz jako astronauta w Nasa,
jako jeden z wielu astronautów NASA zostałeś wybrany do lotu testowego na planetę Mars.
Miesiące mijały na ciężkich treningach i symulacjach.
Misja "New Horizon" w której uczestniczysz przewiduje wysłanie jednego członka załogi na okres 31 dni.
Jego cel to przetestowanie w ekstremalnych warunkach wszystkich systemów podtrzymujących życia,
jak i pobranie próbek gruntu, a następnie wykonanie kilku eksperymentów.
Podróż na planetę Mars miała wyglądać w następujący sposób: Uzyskanie orbity Ziemi,
na orbicie statek zostanie zatankowany przez dodatkowe pojazdy, które nastepnie wylądują na Ziemi,
Następnie zostanie rozpoczęta faktyczna podróż na orbitę Marsa. Po kilku miesiącach, uzyskałeś orbitę czerwonej planety.
Znajdujesz się na "Statku matce" o nazie "Hermes", używasz
pojazdu MDV (Mars Descent Vehicle, służy do wylądowania na powierzchni planety.) do lądowania.
Po bezpiecznym lądowaniu rozkładasz główną bazę, a następnie do niech wchodzisz...'''))
    print('\n' * 2)
    print_lokalizacja()
    komenda()
